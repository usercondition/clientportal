...
function staffNewClientMessage({ clientLabel, bodySnippet }) {
  ...
  const body = [
    ...
    "This email is sent even when you are not logged in — reply from your mail app (Reply-To is their address) or open the admin inbox.",
    "",
    truncate(bodySnippet, 800),
    "",
    "Reply in the admin inbox (Admin → Inbox).",
    portalUrl() ? `Open inbox: ${baseUrl()}/admin-inbox.html` : "",
  ]
    .filter(Boolean)
    .join("\n");
...
function clientNewStaffMessage({ firstName, bodySnippet }) {
  ...
  const body = [
    `Hi ${firstName || "there"},`,
    "",
    "We sent a reply in your client portal thread. This email is delivered even when you are not on the website — open the link below to read the full message and any attachments.",
    "",
    truncate(bodySnippet, 800),
    "",
    portalLinkLine(),
    "",
    "—",
    "You can reply by email if your provider supports it, or sign in to the portal to continue the conversation.",
  ].join("\n");
  return { subject: subj, text: body };
}

/** Staff: client submitted a new order request from the portal. */
function staffNewOrderRequest({ clientLabel, clientEmail, orderNumber, title, summarySnippet }) {
  const subj = `New order request ${orderNumber}`;
  const body = [
    `${clientLabel} submitted a new order request.`,
    clientEmail ? `Email: ${clientEmail}` : "",
    `Reference: ${orderNumber}`,
    `Title: ${title}`,
    "",
    "Details:",
    truncate(summarySnippet, 2000),
    "",
    portalUrl() ? `Open admin dashboard: ${baseUrl()}/admin.html` : "",
  ]
    .filter(Boolean)
    .join("\n");
  return { subject: subj, text: body };
