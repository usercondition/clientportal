...
/** Statements from `003_marketplace_sync.sql` (marketplace_threads / marketplace_messages). */
function getMarketplaceSyncStatements() {
  return parseMigrationFile("003_marketplace_sync.sql");
}

/** Statements from `004_order_quote_flow.sql` (quote item + payment review fields). */
function getOrderQuoteFlowStatements() {
  return parseMigrationFile("004_order_quote_flow.sql");
}

/** Statements from `005_shop_products.sql` (licensed mini catalog). */
function getShopProductsStatements() {
  return parseMigrationFile("005_shop_products.sql");
}

/** Statements from `006_shop_orders.sql` (Stripe shop order records). */
function getShopOrdersStatements() {
  return parseMigrationFile("006_shop_orders.sql");
}

/** Full migrate order: bootstrap + compatibility + marketplace tables. */
function getAllMigrationStatements() {
  return [
    ...getMigrationStatements(),
    ...getCompatStatements(),
    ...getMarketplaceSyncStatements(),
    ...getOrderQuoteFlowStatements(),
    ...getShopProductsStatements(),
    ...getShopOrdersStatements(),
  ];
}
...
/**
 * Apply `003_marketplace_sync.sql` on every startup (idempotent).
 * @param {import("pg").Pool} pool
 */
async function applyMarketplaceSyncSchema(pool) {
  const statements = getMarketplaceSyncStatements();
  if (statements.length === 0) return { ran: false };
...
/**
 * Apply `004_order_quote_flow.sql` on every startup (idempotent).
 * @param {import("pg").Pool} pool
 */
async function applyOrderQuoteFlowSchema(pool) {
  const statements = getOrderQuoteFlowStatements();
  if (statements.length === 0) return { ran: false };
...
/**
 * Apply `005_shop_products.sql` on every startup (idempotent).
 * @param {import("pg").Pool} pool
 */
async function applyShopProductsSchema(pool) {
  const statements = getShopProductsStatements();
  if (statements.length === 0) return { ran: false };
...
/**
 * Apply `006_shop_orders.sql` on every startup (idempotent).
 * @param {import("pg").Pool} pool
 */
async function applyShopOrdersSchema(pool) {
  const statements = getShopOrdersStatements();
  if (statements.length === 0) return { ran: false };
