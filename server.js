...
function saveShopPriceOverrides(overrides) {
  ...
  fs.writeFileSync(SHOP_PRICE_OVERRIDE_PATH, data, "utf8");
}
...
app.get("/api/shop/mix-listings", async (_req, res) => {
  try {
    const overrides = loadShopPriceOverrides();
    const items = getShopInventoryItems().map((item) => {
      ...
      return {
        ...
        vendor: item.vendor || "",
        vendorLabel: item.vendorLabel || item.vendor || "",
        image: item.image || "",
    ...
    console.error("[shop/mix-listings] failed:", err.message);
    return res.status(500).json({ error: "Failed to load mixed shop listings." });
  }
});
